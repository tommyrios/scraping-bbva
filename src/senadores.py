...
class ScrapearSenado:
    ...
    def _normalizar_nombre_key(self, nombre: str) -> str:
        ...
        - quita acentos
        - MAYUS
        - saca puntos y caracteres raros
        - convierte 'APELLIDO, Nombre' -> 'APELLIDO NOMBRE'
        """
    ...
    # ------------------------
    # Mapa senadores (provincia/partido)
    # ------------------------
    def obtener_diccionario_partidos(self):
        ...
        try:
            ...
            WebDriverWait(self.driver, 20).until(
                lambda d: "<table" in (d.page_source or "").lower()
            )
            soup = BeautifulSoup(self.driver.page_source, "html.parser")

            tabla = soup.find("table", id="senadoresTabla") or soup.find("table")
            if not tabla:
                print("No se encontró tabla de senadores.")
                return
            ...
            for tr in tbody.find_all("tr"):
                tds = tr.find_all("td")
                # Estructura real: [Foto, Nombre, Distrito, Partido, Período, Contacto]
                if len(tds) < 4:
                    continue

                nombre = self.limpiar_texto(tds[1].get_text(" ", strip=True))
                provincia = self.limpiar_texto(tds[2].get_text(" ", strip=True))
                partido = self.limpiar_texto(tds[3].get_text(" ", strip=True))

                key = self._normalizar_nombre_key(nombre)
                if key:
                    self.mapa_datos_senadores[key] = {"provincia": provincia, "partido": partido}
                    cargados += 1

            print(f"Senadores mapeados: {cargados}")

        except Exception as e:
            print(f"Error mapeando senadores: {e}")
            return

    # ------------------------
    # Parse listado proyectos
    # ------------------------
    ...
    # ------------------------
    # Parse detalle proyecto
    # ------------------------
    def _get_autores(self, soup_det: BeautifulSoup):
        autores_div = soup_det.find("div", {"role": "tabpanel", "id": "Autores"}) \
                     or soup_det.find(id=re.compile(r"^Autores$", re.IGNORECASE))
        if not autores_div:
            return ["S/D"]
    ...
    def _get_fecha_inicio(self, soup_det: BeautifulSoup):
        h2 = soup_det.find(
            ...
            and "mesa de entradas" in tag.get_text(strip=True).lower()
        )
        if not h2:
            return "S/D"
    ...
    def _get_comisiones(self, soup_det: BeautifulSoup):
        h2 = soup_det.find(
            ...
            and "giros del expediente a comisiones" in tag.get_text(strip=True).lower()
        )
        if not h2:
            return "S/D"
        tabla = h2.find_next("table")
    ...
    def scrape(self):
        ...
        for it in items:
            info = self.extraer_detalle_proyecto(
                it["url_detalle"],
                fallback_extracto=it.get("extracto", "S/D")
            )
            if not info:
                continue
            ...
            autor_key = self._normalizar_nombre_key(autor_para_buscar)

            datos_extra = self.mapa_datos_senadores.get(
                autor_key,
                {"partido": "", "provincia": ""}
            )

            # Debug opcional (ayuda mucho)
            # if not datos_extra.get("provincia") and autor_key:
            #     print(f"[WARN] No match para autor: '{autor_para_buscar}' (key='{autor_key}')")
