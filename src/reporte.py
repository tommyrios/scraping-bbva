...
"""
 ...
 def render(
    ...
    - Ordena los ítems por prioridad: ALTO primero, luego MEDIO
    """
    ...
    # mapping para ordenar por impacto
    ...
    for idx, sec in enumerate(secciones_visibles):
        ...
        # --- FILTRAR y ORDENAR items por impacto (ALTO -> MEDIO) ---
        items_email = []
        for p in items:
            try:
        ...
        # Orden estable por el mapa; si querés, podés añadir secundario por fecha después
        items_ordenados = sorted(items_email, key=lambda x: orden_impacto.get(impacto_normalizer(x), 99))

        # Render items (ahora en orden)
